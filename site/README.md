...
顶部「✦ 闪过学习」按当前章节词表顺序浏览，每组最多 10 词，每词 8 秒。
弹窗展示拼写、音标、词性、释义、例句及可用的词根信息。熟词从本轮排除；
生词、模糊和超时未选都保留，整组结束后进入这些词的拼写训练，不混入已有词本。
全部选熟词时可结束或再学一组。浏览不计学习日历，也不自动修改生词本。
弹窗期间原答题倒计时暂停，退出恢复原训练；取消不推进组次。
组次按用户和章节在本次页面会话内记录，刷新重置，章节末尾后再次从头开始。
流程回归：`node build/test-flash.mjs`。
